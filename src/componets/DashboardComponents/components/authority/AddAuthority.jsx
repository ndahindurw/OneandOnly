...
const AddAuthority = ({ data, url }) => {
  ...
  const fetchUserSuggestions = async (input) => {
    try {
      const response = await axiosInstance.get(`${process.env.REACT_APP_USER_SUGGESTIONS_URL}?input=${input}`);
  ...
  const handleAuthority = async (e) => {
    ...
    try {
      if (selectedUser) {
        const response = await axiosInstance.post(process.env.REACT_APP_ADD_ROLE, {
    ...
    <div className="auth-container">
      <div className="Auth-form-container">
        <form className="form-container" onSubmit={handleAuthority}>
          ...
          <select name="authority" onChange={(e) => setCredentials({ ...credentials, authority: e.target.value })}>
          ...
          {isUserListVisible && suggestedUsers.length > 0 && (
           ...
           {isUserListVisible && UserList && UserList.length > 0 &&
            UserList
              .filter((item) =>
                userInp.trim() === '' ? true :
                  Object.values(item).some((value) =>
                    typeof value === 'string' && value.toLowerCase().includes(userInp.toLowerCase())
                  ))
              .length > 0 && (
                <div>
                  {UserList
...
{selectedUser && (
          ...
          <button type="submit" className="green-btn" disabled={UserList && UserList.length > 0 &&
