...
const CardForm = ({ roomNames, closeRoom }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("My Stored Toke", storedToken);
  ...
  const handleChanges = (e) => {
    ...
    const room = roomNames.find((room) => room.roomNameID === value);
    if (room) {
 ...
 const handleBooking = async () => {
  ...
  try {
    ...
    const bookingPayload = {
      room: {
        roomID: parseInt(selectedRoom.roomNameID)
      },
      user: {
    ...
    if (error.response) {
      ...
      setError("No response from the server.it  can be another  isue.");
    } else {
...
console.log("roooooooooom", roomNames);
...
return (
      ...
      <h3 style={{display:'flex', justifyContent:"center" ,marginBottom:40}}>Room Request</h3>
        ...
        {/* <img
          ...
          alt="Room"
          className="card-image col-xlg-4"
        /> */}
